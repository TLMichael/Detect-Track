...
int main(int argc, char * argv[])
{
    ...
    while ( capture.read(frame) )
    {
        ...
        if (sent_serial)
        {
            if(status == 1)
            {
                int px = location.x + location.width / 2;
                int py = location.y + location.height / 2;
                px -= frame.cols / 2;
                py -= frame.rows / 2;
                string buff = to_string(1) + ' ' + to_string(px) + ' ' + to_string(py) + '\n';
                sel.writeData(buff.data(), (int)buff.length());
            }
            else
            {
                string buff = to_string(0);
                sel.writeData(buff.data(), (int)buff.length());
            }
            waitKey(0);
        }
